...
var knownLibs = []infraLib{
	{"spring-boot-starter-data-redis", "redis", 6379, "Redis (Spring Boot starter)", "cache"},
	{"jedis", "redis", 6379, "Redis (Jedis client)", "cache"},
	{"lettuce-core", "redis", 6379, "Redis (Lettuce client)", "cache"},
	{"kafka-clients", "kafka", 9092, "Kafka (clients)", "broker"},
	{"spring-kafka", "kafka", 9092, "Kafka (Spring)", "broker"},
	{"postgresql", "postgresql", 5432, "PostgreSQL", "database"},
	{"spring-boot-starter-data-jpa", "postgresql", 5432, "PostgreSQL (Spring JPA)", "database"},
	{"mysql-connector-java", "mysql", 3306, "MySQL", "database"},
	{"mysql-connector-j", "mysql", 3306, "MySQL", "database"},
	{"mongo-java-driver", "mongodb", 27017, "MongoDB", "database"},
	{"spring-boot-starter-data-mongodb", "mongodb", 27017, "MongoDB (Spring)", "database"},
	{"spring-boot-starter-amqp", "rabbitmq", 5672, "RabbitMQ (Spring AMQP)", "broker"},
	{"amqp-client", "rabbitmq", 5672, "RabbitMQ", "broker"},
	{"elasticsearch-rest-high-level-client", "elasticsearch", 9200, "Elasticsearch", "search"},
	{"spring-data-elasticsearch", "elasticsearch", 9200, "Elasticsearch (Spring Data)", "search"},
}
		...
		for _, lib := range knownLibs {
			if d.ArtifactID == lib.artifactID {
				deps = append(deps, model.NetworkDependency{
					...
					SourceFile:   path,
					EvidenceLine: fmt.Sprintf("%s:%s", d.GroupID, d.ArtifactID),
					ServiceType:  lib.serviceType,
				})
				break
			}
		}
